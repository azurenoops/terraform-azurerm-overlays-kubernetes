resource "azurerm_resource_group" "aks_rg" {
  name     = "tf-noops-gov-rg-aks-udr-kn"
  location = module.mod_azure_region_lookup.location_cli
}

resource "azurerm_virtual_network" "aks_vnet" {
  name                = "vnet-aks-udr-kn"
  location            = module.mod_azure_region_lookup.location_cli
  resource_group_name = azurerm_resource_group.aks_rg.name
  address_space       = ["10.1.0.0/16"]
}

resource "azurerm_subnet" "aks_subnet_udr_kn" {
  name                 = "snet-aks-udr-kn"
  resource_group_name  = azurerm_resource_group.aks_rg.name
...
resource "azurerm_subnet" "aks_subnet_udr_ingress" {
  name                 = "snet-aks-udr-kn-ingress"
  resource_group_name  = azurerm_resource_group.aks_rg.name
  virtual_network_name = azurerm_virtual_network.aks_vnet.name
...
resource "azurerm_lb" "ingress_aks_lb" {
  ...
  frontend_ip_configuration {
    ...
    subnet_id                     = azurerm_subnet.aks_subnet_udr_ingress.id
...
resource "azurerm_virtual_network_peering" "hub_to_aks_spoke" {
  name                      = "hub_to_aks_spoke_kn"
  allow_forwarded_traffic             = true
  allow_virtual_network_access        = true
  #allow_remote_virtual_network_access = true
  resource_group_name       = "tf-anoa-usgaz-hub-core-dev-rg"
  virtual_network_name      = "tf-anoa-usgaz-hub-core-dev-vnet"
  remote_virtual_network_id = "/subscriptions/df79eff1-4ca3-4d21-9c6b-64dd15c253e8/resourceGroups/tf-noops-gov-rg-aks-udr-kn/providers/Microsoft.Network/virtualNetworks/vnet-aks-udr-kn"
}

resource "azurerm_log_analytics_workspace" "aks" {
  name                = "aks-la-01-udr-kn"
